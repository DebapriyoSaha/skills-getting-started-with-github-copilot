...
def test_signup_duplicate():
    activity = "Chess Club"
    test_email = "duplicateuser@mergington.edu"
    # Ensure clean state
    client.delete(f"/activities/{activity}/unregister", params={"email": test_email})
    # First signup
...
def test_unregister_not_found():
    activity = "Chess Club"
    test_email = "notfounduser@mergington.edu"
    response = client.delete(f"/activities/{activity}/unregister", params={"email": test_email})
...
def test_signup_activity_not_found():
    response = client.post("/activities/NonexistentActivity/signup", params={"email": "nouser@mergington.edu"})
    assert response.status_code == 404
    assert "Activity not found" in response.json()["detail"]


def test_unregister_activity_not_found():
    response = client.delete("/activities/NonexistentActivity/unregister", params={"email": "nouser@mergington.edu"})
    assert response.status_code == 404
    assert "Activity not found" in response.json()["detail"]
